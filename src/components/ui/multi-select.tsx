...
import { cn, getSanitizedName, isValueInDbRecipients } from "@/lib/utils";
...
>(({ className, children, ...props }, ref) => {
  const { value, onValueChange, activeIndex, customPayload } = useMultiSelect();
  ...
  return (
    ...
    >
      {value?.map((item, index) =>
        item !== "all-stakeholders" && item !== "trash-stakeholders" ? (
          <Badge
            key={item}
            className={cn(
            ...
            variant={
              isValueInDbRecipients(item, customPayload)
                ? "warning"
                : "secondary"
            }
          >
            <span className="text-xs">{getSanitizedName(item) || item}</span>
            <button
            ...
            >
              ...
              <CloseIcon className="h-4 w-4 hover:stroke-destructive" />
            </button>
          </Badge>
        ) : null,
      )}
