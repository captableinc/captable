...
export const inviteMemberProcedure = adminOnlyProcedure
  ...
  .mutation(async (args) => {
    return inviteMemberProcedureHandler(args);
  });

interface inviteMemberProcedureHandlerOptions {
  input: TypeZodInviteMemberMutationSchema;
  ctx: TypeAdminOnlyTRPCContext;
}

async function inviteMemberProcedureHandler({
  ctx,
  input,
}: inviteMemberProcedureHandlerOptions) {
